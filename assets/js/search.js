(() => {
  ...
  const tom_select = new TomSelect("#cerca-concepte", {
    ...
    onType: function (query) {
      ...
      if (query.length) {
        ...
        // 2. Those that contain the normalized query (but do not start with it).
        ...
        // Combine the two lists, prioritizing options that start with the query.
  ...
  window.addEventListener("pageshow", function () {
    // If it came from searching a concept, focus that element.
    ...
    // By default, focus the search by sentence element.
    frase.focus();

    // On desktop, select the searched value, so it can be replaced by simply typing.
    if (frase.value && !/Android|iPad|iPhone/i.test(navigator.userAgent)) {
      frase.select();
    }
  });
})();
